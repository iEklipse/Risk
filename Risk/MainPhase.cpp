...
void MainPhase::attack(Player* attacker, map<string, Country*> allCountries, vector<Player*> players) 
{
	...
	for (Country * c : attacker->getCountriesThatCanAttack())
	{
		for (int i = 0; i < c->getBoarders().size(); i++)
			if (c->getBoarders()[i]->getOwner() != attacker->getPlayerName())
			{
				canIssueAttack = true;
				break;
			}
	...
	if (canIssueAttack)
	{
		...
		while (moreAttacks)
		{
			cout << "Here is the list of countries that can attack and a list of borders that your countries can invade: " << endl;
			for (Country * c : attacker->getCountriesThatCanAttack())
			{
				cout << "- " << c->getName() << ": " << c->getArmySize() << " armies" << endl;
				for (int i = 0; i < c->getBoarders().size(); i++)
					if (c->getBoarders()[i]->getOwner() != attacker->getPlayerName())
						cout << "\t >> " << c->getBoarders()[i]->getName() << " (" << c->getBoarders()[i]->getArmySize() << " armies)" << endl;
			}
			...
			battle.attack(attacker, attackingCountry, defendingPlayer, defendingCountry);
	...
	{
		cout << "You currently do not have any Country that can attack. Skipping Attack Phase." << endl;
	}
}

void MainPhase::fortification(Player* player, World* world) 
{
	...
	for (Country * c : player->getListCountriesOwned())
	{
		if (c->getArmySize() >= 2)
		{
			for (Country* border : c->getBoarders())
			{
				if (border->getOwner() == player->getPlayerName())
				{
					canFortify = true;
					break;
				}
	...
	{
		cout << "All of your Countries only have one army. No country can be Fortified. Skipping Fortification Phase." << endl;
	}
...
void MainPhase::moveArmies(Player* player, World* world)
{
	...
	if (fortify)
	{
		...
		for (int i = 0; i < player->getListCountriesOwned().size(); i++)
			cout << vectorCountries[i]->getName() << " (" << vectorCountries[i]->getArmySize() << " armies), ";
		...
		{
			bool canMove = false;
			if (vectorCountries[i]->getArmySize() > 1)
			{
				for (Country* border : vectorCountries[i]->getBoarders())
				{
					if (border->getOwner() == player->getPlayerName())
					{
						canMove = true;
				...
				if (canMove)
				{
					cout << vectorCountries[i]->getName() << " to " << endl;
		...
		cout << "You can move between 1 and " << c->getArmySize() - 1 << " armies." << endl;
		cout << "\nHow many armies you want to move from that country? ";
		cin >> numberOfArmies;
	
		while (!cin || (numberOfArmies < 1 && numberOfArmies > c->getArmySize()-1)) {
			cout << "\nHow many armies you want to move? ";
			cin.clear();
			cin.ignore(256, '\n');
			cin >> numberOfArmies;
		}
